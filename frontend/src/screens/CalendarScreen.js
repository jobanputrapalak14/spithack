...
//   // ⭐ Added fetchGoogleCalendarEvents here
//   const { tasks, theme, googleCalendarEvents, googleTokens, googleLoading, fetchGoogleCalendarEvents } = useApp();
...
//   // ⭐ ADDED THIS BLOCK: Fetches fresh events every time the screen comes into focus
//   // useFocusEffect(
//   //   useCallback(() => {
//   //     if (googleTokens?.accessToken && fetchGoogleCalendarEvents) {
//   //       fetchGoogleCalendarEvents();
//   //     }
//   //   }, [googleTokens, fetchGoogleCalendarEvents])
//   // );
//   useFocusEffect(
//     useCallback(() => {
//       if (googleTokens?.accessToken && fetchGoogleCalendarEvents) {
//         fetchGoogleCalendarEvents();
//       }
//     }, []) // 🔥 EMPTY ARRAY: Only fetch when the screen is opened!
//   );
...
import { Feather as Icon } from '@expo/vector-icons';
import { LinearGradient } from 'expo-linear-gradient';
import { useIsFocused } from '@react-navigation/native'; // ⭐ CHANGED THIS
import { useApp } from '../context/AppContext';
...
export default function CalendarScreen({ navigation }) {
  ...
  // ⭐ SAFE TAB-SWITCH REFRESH (NO MORE INFINITE LOOPS)
  useEffect(() => {
    // This will only run when isFocused flips from false to true (when you click the tab)
    if (isFocused && googleTokens?.accessToken) {
      fetchGoogleCalendarEvents();
    }
  }, [isFocused]);
            ...
            <View style={[styles.googleSection, { backgroundColor: colors.calendarCard }]}>
              ...
              <View style={styles.googleHeader}>
                ...
                <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                  {/* ⭐ ADDED MANUAL REFRESH BUTTON FOR GREAT UX */}
                  <TouchableOpacity
                    onPress={() => fetchGoogleCalendarEvents()}
                    style={{ marginRight: 15 }}
                    activeOpacity={0.7}
                  >
                    <Icon name="refresh-cw" size={18} color="#4285F4" />
                  </TouchableOpacity>
